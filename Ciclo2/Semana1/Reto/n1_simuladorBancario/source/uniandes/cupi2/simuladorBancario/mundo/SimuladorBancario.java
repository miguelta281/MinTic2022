...
public class SimuladorBancario
{
    ...
    {
    	mesActual += 1;
        saldoAntes = corriente.darSaldo( ) + ahorros.darSaldo( ) + inversion.calcularValorPresente( mesActual - 1 );
        ahorros.actualizarSaldoPorPasoMes( );
        interesGenerado = interesGenerado + (calcularSaldoTotal() - saldoAntes);
    }
    ...
    {	
    	saldoAntes = corriente.darSaldo( ) + ahorros.darSaldo( ) + inversion.calcularValorPresente( mesActual - pMeses);
    	ahorros.actualizarPorMeses( pMeses );
    ...
    {	
    	...
    	interesGenerado = 0; 
    	mesActual = 1; 
    	cerrarCDT (); // Cierra el cdt 
    	retirarCuentaCorriente(corriente.darSaldo()); // Retira lo que se paso del cdt a la corriente
        retirarCuentaAhorros(ahorros.darSaldo()); // Retira el saldo de la cuenta de ahorros
    	return interesTotal;
    }
}