...
public class TiendaDeLibros
{
    ...
    //--------------------------------------------------------------------
    // Contador de transacciones (ventas)
    private int numeroTrn;
    // -----------------------------------------------------------------
    // Constructor
    // -----------------------------------------------------------------
    ...
    public boolean vender( String pIsbn, int pCantidad, String pFecha )
    {
        ...
        if( buscado != null )
        {
            ...
            // Aumenta la caja con el valor total de los ejemplares vendido
    ...
    {	
    	//Recorrer el catalogo, y obtener el precio de cada libro
    	for (int i= 0; i < darCatalogo().size(); i++ )
    	{
    		 Libro actual = catalogo.get( i );
    		 double precio =  actual.darPrecioVenta();
    		 for (int j = 0; j < lim_inf.length; j++)
    		 {
    			// System.out.println(precio + "entre" + lim_inf[j] +  "y" +  lim_sup[j]);
    			 if(precio >= lim_inf[j] && precio < lim_sup[j])
    			 {
    				 double precio_descuento = precio  - (precio * descuentos[j]);
    				 actual.cambiar_Precio_Venta(precio_descuento);
    				 darCatalogo().set(i,actual);
    			 }
    		 }
    		
    	}
    	
    //	System.out.println(darCatalogo().size());
    ...
    public Transaccion metodo2( )
    {
    	...
    	int tamano_catalogo = catalogo.size();
    	
    	ArrayList<Transaccion> trn_libro_actual = new ArrayList<Transaccion>();
    	
    	Transaccion trn_actual = null;
    	double precio_actual = 0;
    	int cant_libro = 0;
    	double venta_actual = 0;
    	double mejor_venta = 0;
    	
    	if(numeroTrn !=0)
    	{	
    	for(int i = 0; i < tamano_catalogo; i++)
    	{
    		libroActual = catalogo.get(i);
    		trn_libro_actual =  libroActual.darTransacciones();
    		
    		int tamano_trn_libro_actual = trn_libro_actual.size();
    		
    		for (int j = 0; j < tamano_trn_libro_actual; j++)
    		{
    			trn_actual = trn_libro_actual.get(j); 
    			if(trn_actual.darTipo().equals(Transaccion.Tipo.VENTA))
    			{
    				cant_libro = trn_actual.darCantidad();
    				precio_actual = libroActual.darPrecioVenta();
    				venta_actual = cant_libro * precio_actual;
    				if(venta_actual > mejor_venta)
    				{
    					mejor_venta = venta_actual;
    					ventaMasCara = trn_actual;
    				}
    			}
    		}
    	}		
